...
namespace CustodianAdmin.Data
{
    public class RepairsBillRepository
    {
        ...
        public IList<RepairsBill> RepairsBills()
        {
            ...
            //               .List<RepairsBill>();
            //               //.Where(c => c.EntryDate == DateTime.Today).ToList();

            //    return pDet;
            //}

            string hqlOptions = "from RepairsBill i where i.EntryDate >= '" + DateTime.Now.Date + "'";

            using (var session = GetSession())
            {
                return session.CreateQuery(hqlOptions).List<RepairsBill>();
